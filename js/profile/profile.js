...
function profileHome(){
 ...
 <div class="card profile-login-card"><div class="profile-login-avatar">♙</div><div class="field"><label for="playerLogin">Логин</label><input id="playerLogin" autocomplete="username" placeholder="Ваш логин"></div><div class="field"><label for="playerPassword">Пароль</label><input id="playerPassword" type="password" autocomplete="current-password" placeholder="Ваш пароль" onkeydown="if(event.key==='Enter')submitPlayerLogin()"></div><div class="actions"><button class="primary" onclick="submitPlayerLogin()">Войти</button><button class="ghost" onclick="route('register')">Создать аккаунт</button><button class="ghost" onclick="route('rating')">Рейтинг</button></div></div></section>`,`profile`);
}

function registerProfile(){layout(`<section class="profile-login-page"><div class="eyebrow">LOVE CHESS</div><h1>Создать аккаунт</h1><p>Для публикации игрового профиля нужны ник, логин и пароль.</p><div class="card profile-login-card"><div class="field"><label for="registerNickname">Ник</label><input id="registerNickname" maxlength="40" placeholder="Например RomaChess"></div><div class="field"><label for="registerLogin">Логин</label><input id="registerLogin" autocomplete="username" placeholder="3–32 символа, латиница и цифры"></div><div class="field"><label for="registerPassword">Пароль</label><input id="registerPassword" type="password" minlength="10" autocomplete="new-password" placeholder="Не менее 10 символов"></div><div class="field"><label><input id="privacyConsent" type="checkbox"> Я принимаю <button type="button" class="ghost" onclick="route('privacy')">политику конфиденциальности</button>.</label></div><div class="actions"><button class="primary" onclick="registerPlayer()">Создать аккаунт</button><button class="ghost" onclick="route('profile')">Отмена</button></div></div></section>`,'profile')}
